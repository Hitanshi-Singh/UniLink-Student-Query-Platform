...
const getQuestionsRelatedToUserSubscribedTags = asyncHandler(async (req,res)=>{
  try {
    ...
    const questions = await Question.find({ relatedTags: { $in: user.subscribedTags } }. populate({path: 'answers', 
    ...
    res.status(200).json(new ApiResponse(200, questions, "User's interested questions fetched duccessfully" ));
    }
    catch (error) {
    res.status(500).json(new ApiError(500, "Error has occured" ));
  }
...
export {addQuestion, getAllQuestions, updateQuestion, deleteQuestion, getUserQuestionHistory, getQuestionsRelatedToUserSubscribedTags}
